...
Les appels correspondants sont les méthodes `admin*` de `lib/api.ts` (voir §6).
...
| Données | `StatCard`, `TrophiesSection`, `EloChart`, `WinRateBar` (jauge W/L façon OP.GG, variantes `full`/`compact`), `Palmares` (classements par saison), `Badges` (rendu animé des badges, catalogue `lib/badges.ts`), `RankedBadge` |
...
| Tournoi / OPS | `TournamentCup` (coupe / case carrée d'un tournoi avec image de couverture), `OpsRevealOverlay` (révélation cinématique de la cible d'un OPS) |
...
`LeaderboardScatter` — un **nuage de points** (variante *beeswarm*) zoomable/pannable (ELO en ordonnée,
chaque avatar cliquable mène au profil). Un **sélecteur de saison** (dispo aussi sur mobile) permet de
consulter les classements figés des saisons passées.

Autres pages notables : **`H2HPage`** (`/h2h`) — confrontation directe entre deux joueurs (bilan,
historique des duels) ; **`GoatPage`** (`/goat`) — palmarès all-time et saisons. Le profil joueur
(`PlayerPage`) affiche badges, palmarès et un bouton **suivre / préférences de notif**.
