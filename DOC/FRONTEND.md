...
`ops`, `tournaments` (CRUD + record/confirm), profils, `locations`, et les routes admin
(`adminUsers`, `adminSetStats`, `adminBan/Unban`, `adminDeleteMatch`, `adminEditMatch`,
`adminRejectedMatches`, `adminSuspicious`, `adminAuditLog`), `featureRequests`.
...
Deux dictionnaires en dur : **fr** et **en**. `I18nProvider` stocke `lang` (persisté dans
`localStorage['league:lang']`, fallback langue navigateur sinon fr). `useI18n()` / `t(key)` renvoie
la chaîne traduite (fallback fr si clé absente).
...
| Données | `StatCard`, `TrophiesSection`, `EloChart` |
...
| Divers | `ErrorBoundary`, `FeatureRequestBox` |
