...
export function winnerTeam(t: Tournament): WinnerTeam | null {
  ...
  const capImg = t.winner?.imageUrl ?? null;
  if (!is2v2) return { is2v2, label: login, members: [{ login, imageUrl: capImg }] };
  const e = (t.entries ?? []).find((x) => x.login === login || x.partnerLogin === login);
  if (!e) return { is2v2, label: login, members: [{ login, imageUrl: capImg }] };
  const members: { login: string; imageUrl: string | null }[] = [
    { login: e.login, imageUrl: e.user?.imageUrl ?? capImg },
  ];
  if (e.partnerLogin) {
