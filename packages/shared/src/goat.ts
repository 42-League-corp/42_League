// ─── Score G.O.A.T — classement « meilleur joueur de tous les temps » ─────────
//
// Module PUR (aucune dépendance, aucun accès base) afin d'être importable côté
// backend (attribution du badge `goat`) ET côté web (page G.O.A.T). Le calcul
// agrège toutes les stats « positives » d'un joueur en une note unique, pondérée.
// L'idée : le meilleur joueur de tous les temps n'est pas forcément le n°1 à
// l'ELO — un joueur qui gagne de gros écarts, enchaîne les séries et remporte des
// tournois officiels peut le coiffer.
//
// Pondération demandée explicitement :
//   - ELO              40 %  (le plus lourd)
//   - Tournois         20 %  (officiels 16 % + amicaux 4 %)
//   - Le reste (40 %) réparti entre goal average, écart en victoire, séries de
...
export interface GoatPlayer<E extends GoatEntry = GoatEntry> {
  entry: E;
  metrics: GoatMetrics;
...
// ── Formes d'entrée minimales (structurelles) ────────────────────────────────
// Le front passe ses LeaderboardEntry / PlayedMatch / Tournament ; le backend
// passe des projections Prisma. On ne type ici que les champs réellement lus.

export interface GoatEntry {
  login: string;
}

export interface GoatMatchInput {
  countedForElo?: boolean | null;
  playerALogin: string;
  playerBLogin: string;
  deltaA?: number | null;
  deltaB?: number | null;
...
export interface GoatTournamentInput {
  status: string;
...
/**
 * Calcule le classement G.O.A.T à partir du leaderboard (ensemble des candidats),
 * de l'historique des matchs (buts, marges, séries) et des tournois (titres
 * officiels / amicaux). `ranking[0]` est le G.O.A.T (#1).
 */
export function computeGoat<E extends GoatEntry>(
  leaderboard: E[],
  matches: GoatMatchInput[],
  tournaments: GoatTournamentInput[],
): GoatPlayer<E>[] {
  const known = new Set(leaderboard.map((e) => e.login));

  // ELO cumulé « all-time » : le GOAT doit rester PERSISTANT entre les saisons.
  // Or l'ELO du leaderboard est remis au plancher de palier à chaque clôture de
  // saison → il ne reflète que la saison courante. On reconstruit donc un ELO
  // continu en repartant de l'ELO de départ (1000) et en ré-appliquant la
  // variation de CHAQUE match de toutes les saisons (l'historique n'est jamais
  // purgé). Les nulles (échecs) comptent aussi car elles bougent l'ELO.
  ...
  // Métriques brutes par joueur candidat.
  ...
  const scored: GoatPlayer<E>[] = rows.map(({ entry, metrics }) => {
